...
describe('Server operation: ', () => {
  ...
  test('get server informatom', async () => {
    ...
    expect(Object.keys(response.data).sort()).toEqual(
      sortedArrayByAlph(['version', 'uptime'])
    );
  ...
  test('sum', async () => {
    const response = await instance
      .get(
        `/operation/${OPERATION_NAME.SUMMATION}?firstOperand=${firstSumOperand}&secondOperand=${secondSumOperand}`
      )
      .catch(e => e);

    expect(response.status).toBe(200);
    expect(Object.keys(response.data).sort()).toEqual(
      sortedArrayByAlph(['firstOperand', 'secondOperand', 'total'])
    );
  ...
  test('get last operation', async () => {
    ...
    expect(response.status).toBe(200);
    expect(Object.keys(response.data).sort()).toEqual(
      sortedArrayByAlph(['lastOperationName'])
    );
  ...
  test('multiplication', async () => {
    const response = await instance
      .get(
        `/operation/${OPERATION_NAME.MULTIPLICATION}?firstOperand=${firstMultOperand}&secondOperand=${secondMultOperand}`
      )
      .catch(e => e);
    ...
    expect(response.data.firstOperand).toBe(firstMultOperand);
    expect(response.data.secondOperand).toBe(secondMultOperand);
    expect(response.data.total).toBe(firstMultOperand * secondMultOperand);
  ...
  test('total operation information DESC', async () => {
    const response = await instance
      .get(`/operation/history?sort=DESC`)
      .catch(e => e);

    expect(response.status).toBe(200);
    expect(Object.keys(response.data).sort()).toEqual(
      sortedArrayByAlph(['history'])
    );
    ...
    expect(Object.keys(firstOperation).sort()).toEqual(
      sortedArrayByAlph([
        'operationName',
        'firstOperand',
        'secondOperand',
    ...
    expect(Object.keys(secondOperation).sort()).toEqual(
      sortedArrayByAlph([
        'operationName',
        'firstOperand',
        'secondOperand',
  ...
  test('total operation information ASC', async () => {
    const response = await instance
      .get(`/operation/history?sort=ASC`)
      .catch(e => e);
